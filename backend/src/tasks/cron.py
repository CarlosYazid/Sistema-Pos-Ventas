...
from models.alert import StockAlert, ExpirationAlert
...
def low_stock_alerts():
    ...
    with Session(get_sync_engine()) as session:
        prods= (
            session.query(StockAlert)
            .where(StockAlert.notified == False)
            .all())
        
    if not prods:
        return
        
    asyncio.run(EMAIL_SERVICE.send_message(
        message=MessageSchema(
            subject = f"Alerta: productos con stock bajo",
...
def expired_alerts():
    ...
    with Session(get_sync_engine()) as session:
        prods= (
            session.query(ExpirationAlert)
            .where(ExpirationAlert.notified == False)
            .all())
    ...
    asyncio.run(EMAIL_SERVICE.send_message(
        message=MessageSchema(
            subject = f"Alerta: productos vencidos",
            recipients = [SETTINGS.company_email],
            template_body = {
                'company': {
                    'name': SETTINGS.company_name,
